...
ServerConnectWindow::ServerConnectWindow()
{
	...
	autoReconnect = false;
	status = "空闲";
	lastConnected = GetSocketClient().IsConnected();
...
void ServerConnectWindow::drawConnectionControls()
{
	...
	{
		if (ImGui::Button("连接")) {
			bool ok = client.Connect(hostBuf, static_cast<uint16_t>(port));
			updateStatus(ok, "连接");
		}
	}
	else
...
void ServerConnectWindow::drawDriverControls()
{
	...
	if (ImGui::Combo("##kernelVersion", &currentKernelVersion, kernelVersionList, IM_ARRAYSIZE(kernelVersionList))) {
